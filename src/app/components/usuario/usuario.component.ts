...
export class UsuarioComponent implements OnInit{
  ...
  constructor(private router: Router, private usuarioService: UsuariosService, private routerIntParams: ActivatedRoute, private toastr: ToastrService, private fb: FormBuilder) {
  ...
  getUsuarios() {
    this.routerIntParams.params.subscribe(params => {
      ...
      try {
        this.usuarioService.index(page).pipe(retry(1), catchError(this.handleError)).subscribe({
          next: (data) => {
            this.usuarios = data
          ...
          error: (error) => {
            if (error == 401) {
              this.mensaje = "No tiene permiso para ingresar a la lista de usuarios";
            } if (error == 400) {
  ...
  registrarUsuario() {
    ...
    this.usuarioService.crear(formData, 1).pipe(retry(1), catchError(this.handleError)).subscribe({
      next: (data) => {
      },
      error: () => {
        this.alerta = true;
  ...
  actualizarUsuario() {
    ...
    try {
      ...
      if (this.tipoActualizacion === 'editarCliente') {
        actualizarObservable = this.usuarioService.actualizarCliente(formData, tipo);
      } else {
        actualizarObservable = this.usuarioService.actualizar(formData, tipo);
      }
  
      // Realizar la solicitud de actualización
      actualizarObservable.pipe(retry(1), catchError(this.handleError)).subscribe({
        next: (data) => {
          console.log(data);
          this.getUsuarios(); 
  ...
  cambiarStateImg(usuario: any) {
    this.imagenState = false;
    this.usuario = new Usuario(usuario.correo, usuario.roles, usuario.password, usuario.nombre, usuario.dni, usuario.id);
  }
  ...
  verUsuario(usuario: any, tipo: string) {
    ...
    this.usuarioEditar = new Usuario(usuario.correo, usuario.roles, '', usuario.nombre, usuario.dni, usuario.id);
    ...
    this.usuarioForm.patchValue({
      ...
      nombre: usuario.nombre,
      dni: usuario.dni,
      roles: usuario.roles && usuario.roles.length > 0 ? usuario.roles[0].id : null,
      password: '',
      id: usuario.id
    });
  ...
  eliminarUsuario(usuario: Usuario) {
    if (confirm('¿Estás seguro de que quieres eliminar a este usuario?')) {
      ...
      try {
        this.usuarioService.eliminar(usuario).pipe(retry(1), catchError(this.handleError)).subscribe({
          next: (data) => {
            console.log(data);
            this.getUsuarios();
          },
...
isRoleSelected(roleId: number): boolean {
  return Array.isArray(this.usuarioEditar.roles) && this.usuarioEditar.roles.some((role: Roles) => role.id === roleId);
}
